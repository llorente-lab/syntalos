...
// Basically: just two checkboxes -> start IR stream or start Depth Stream. By default, depth stream is set to true and
// IR stream is set to false
...
public:
    ...
    {
        return m_depthStreamCheckBox->isChecked();
    }
    bool isIRStreamEnabled() const
    {
        return m_irStreamCheckBox->isChecked();
    }

    void setDepthStreamEnabled(bool enabled)
    {
        m_depthStreamCheckBox->setChecked(enabled);
    }
    void setIRStreamEnabled(bool enabled)
    {
        m_irStreamCheckBox->setChecked(enabled);
    }
...
private:
    ...
    // IR Stream
    // I don't think we really need to separate this by raw vs display since we don't need infrared videos for MoSeq
    // analysis However, if its more computationally efficient TO separate them, this might make more sense
...
public:
          ...
          m_irStreamEnabled(false)
    {
        ...
        m_depthDispOut = registerOutputPort<Frame>(
            QStringLiteral("depth-disp-out"), QStringLiteral("Display Depth Frames"));
    ...
    // No actual recording here, just getting ready for it
    {
        try {
            ...
            if (!m_depthStreamEnabled && !m_irStreamEnabled) { // Handle case where neither stream is enabled
            ...
            if (m_irStreamEnabled) { // if ir stream is enabled
                auto irProfile = m_pipeline->getStreamProfileList(OB_SENSOR_IR)
                                     ->getVideoStreamProfile(640, OB_HEIGHT_ANY, OB_FORMAT_Y16, 30);
    ...
    void runThread(OptionalWaitCondition *waitCondition) override
    {
        ...
        try {
            ...
            while (m_running) {
                ...
                // make sure this is out of the main loop or else processIRFrame and processDepthFrame will be
                // incrementing frame index simultaneously
                ...
                if (currentFps < (m_fps - 10)) {
                    fpsLow = true;
                    setStatusMessage(
                        QStringLiteral("<html><font color=\"red\"><b>Framerate (%1fps) is too low!</b></font>")
                            .arg(currentFps));
                } else if (fpsLow) {
                    fpsLow = false;
                    statusMessage("Acquiring frames...");
...
private:
    void processDepthFrame(std::shared_ptr<ob::DepthFrame> depthFrame)
    {
        ...
        // OpenCV matrices (cv::Mat) are defined with 4 main parameters: height, width, pixel format, and a pointer to
        // the data.
        // 1. 'height' and 'width' specify the dimensions of the image, which should match the resolution of the depth
        // camera output.
        // 2. We use 'CV_16UC1' for the pixel format, meaning each pixel is represented by a 16-bit unsigned integer
        // ('16U') and the image has one channel ('C1').
        //    This format is ideal for depth data, as depth sensors typically provide high-precision 16-bit depth
        //    values.
        ...
        // By passing the pointer to the data, OpenCV doesn't copy the data; it directly uses the provided memory, which
        // is efficient for real-time processing. We're gonna send this directly to raw depth port This is not processed
        // because processing is being handled by moseq
    ...
    void processIRFrame(std::shared_ptr<ob::IRFrame> irFrame, std::shared_ptr<DataStream<Frame>> &output)
    {
        ...
        try {
            ...
            cv::Mat irVis;
            irMat.convertTo(
                irVis, CV_8UC1, 1.0 / 256.0); // Convert 16-bit to 8-bit which we need to work with the canvas module
